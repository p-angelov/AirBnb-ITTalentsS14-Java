...
public class ValidationUtil {
    ...
    public static void validateEdit(UserEditProfileDto dto,User u){
        if (!ValidationUtil.isValidUsername(dto.getUsername()) ||
            ...
            throw new BadRequestException("The new username is not valid or already exists!");
        }
        if (!ValidationUtil.isValidEmail(dto.getEmail()) ||
                (!ValidationUtil.isEmailFree(dto.getEmail())&& !(dto.getEmail().equals(u.getEmail())))) {
            throw new BadRequestException("The new email is not valid or already exists!");
        }

        if (!ValidationUtil.isValidBirthDate(dto.getDateOfBirth())) {
            throw new BadRequestException("The new date of birth is not valid!");
        }
        if (!ValidationUtil.isValidPhoneNumber(dto.getPhoneNumber())||ValidationUtil.isPhoneNumberFree(dto.getPhoneNumber())) {
            throw new BadRequestException("The new phone number is not valid!");
        }
    }
